...
const FullLibrary: React.FC<FullLibraryProps> = ({ fragrances, toggleFragranceFavorite, isFragranceFavorite, showFragrancePage }) => {
    ...
    const currentItems = filteredFragrances.slice(indexOfFirstItem, indexOfLastItem);
        ...
        <div className='flex flex-col gap-[2rem] box-border h-[90%] w-[103%]'>
            ...
            <div className='custom-scrollbar flex flex-col gap-[2rem] box-border overflow-hidden overflow-y-scroll p-2'>
                ...
                <div className='w-full flex justify-center gap-5'>
                    <button className='text-lg text-neutral-600 hover:text-black dark:text-neutral-300 transition dark:hover:text-white disabled:dark:hover:text-neutral-300 disabled:hover:text-neutral-600' onClick={prevPage} disabled={currentPage === 1}>← Prev</button>
                    <p className='dark:text-white text-lg'>{currentPage}</p>
                    <button className='text-lg text-neutral-600 hover:text-black dark:text-neutral-300 transition dark:hover:text-white disabled:dark:hover:text-neutral-300 disabled:hover:text-neutral-600' onClick={nextPage} disabled={currentPage === totalPages}>Next →</button>
                </div>
            </div>
        </div>
