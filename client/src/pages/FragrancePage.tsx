...
import { IoHeart, IoNewspaperOutline } from "react-icons/io5";
...
const FragrancePage: React.FC<FragrancePageProps> = ({
  fragranceId,
  library,
  onToggleFavorite,
  isFavorite,
  ...
  const heartIconStyle = isFavorite(fragranceId)
  ...
  return (
    <div className="w-[103%] h-full bg-neutral-200 dark:bg-neutral-950 flex flex-col overflow-hidden overflow-y-scroll custom-scrollbar p-1">
      <div className="w-full flex flex-col gap-[1.5rem] pr-6">
        <h1 className="text-3xl font-semibold dark:text-white flex items-center gap-2">
          <IoNewspaperOutline /> Fragance Page
        </h1>
      </div>
      <div className="w-full pt-[50px] mb-[80px] pr-10 flex gap-[2rem] justify-between">
        <div className="w-[60%]  flex flex-col gap-[1rem] justify-between">
          <div>
            <h1 className="text-7xl font-bold dark:text-white">
              {fragrance.name}
            </h1>
            <p className="font-light dark:text-white text-md ">
              <span className="font-light text-md text-blue-700">By</span>{" "}
              {fragrance.brand}
            </p>
          </div>
          ...
          <div className="mt-10">
            <h1 className="text-lg font-normal mb-2 dark:text-neutral-300">
              Fragrance Notes <br />
            </h1>
            <div className="flex gap-1 flex-wrap">
              {fragrance.notes.map((note: string) => {
                return (
                  <p className="box-border p-[5px] rounded-lg w-fit text-xs text-white dark:text-black bg-neutral-900 dark:bg-neutral-100 shadow">
                    {note}
                  </p>
                );
              })}
          ...
          <div className="flex gap-[1rem] mt-[2rem]">
            <button className="w-fit brightness-150 dark:brightness-100 p-[2px] rounded-lg bg-gradient-to-br from-blue-800 via-blue-600 to-blue-800 shadow-md">
              <div className="px-6 py-2 backdrop-blur-xl bg-black/80 rounded-md font-semibold w-full h-full hover:brightness-125">
            ...
            <button
              className=""
              onClick={() =>
                onToggleFavorite(fragranceId, isFavorite(fragranceId))
              }
            >
              {heart}
            </button>
        ...
        <div className="w-[40%]">
          <img
            className="object-cover w-full h-[700px] rounded  border border-neutral-300 dark:border-neutral-900"
            src={fragrance.image}
            alt={fragrance.name}
          />
        </div>
